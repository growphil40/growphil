  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-text-secondary mt-1">Record closed deals and track revenue metrics</p>
      </div>

      {/* Stats Summary */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="rounded-xl border border-border bg-card p-6">
          <p className="text-xs font-medium text-text-secondary uppercase">Total Revenue (Gross Volume)</p>
          <p className="text-3xl font-bold mt-2 text-emerald-500 dark:text-emerald-400">
            ₹{totalRevenue ? totalRevenue.toLocaleString() : '0'}
          </p>
        </div>
        <div className="rounded-xl border border-border bg-card p-6">
          <p className="text-xs font-medium text-text-secondary uppercase">Deals Closed</p>
          <p className="text-3xl font-bold mt-2 text-foreground">{sales.length}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            {loading && <p className="text-text-secondary text-sm">Loading closed deals...</p>}
            
            {!loading && sales.length === 0 && (
              <p className="text-text-secondary text-sm">No sales recorded yet.</p>
            )}
            ...
            <div className="space-y-3">
              {sales.map((sale) => (
                <div
                  key={sale.id}
                  className="p-4 rounded-lg border border-border bg-card-secondary flex justify-between items-center"
                >
                  <div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-text-secondary uppercase">{sale.currency}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="rounded-xl border border-border bg-card p-6 h-fit">
          ...
          <form onSubmit={handleRecordDeal} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-text-secondary mb-2">Select Lead</label>
              <select
                value={leadId}
            ...
            <div>
              <label className="block text-xs font-medium text-text-secondary mb-2">Transaction Value</label>
              <div className="flex gap-2">
                <input
                  type="number"
                  value={amount}
                  onChange={(e) => setAmount(e.target.value)}
                  className="flex-1 rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none"
                  placeholder="50000"
                  disabled={isSubmitting}
                />
                <select
                  ...
                  className="bg-background border border-border rounded-lg px-2 text-sm text-foreground focus:outline-none"
                >
            ...
            <div>
              <label className="block text-xs font-medium text-text-secondary mb-2">Closure Timestamp</label>
              <input
                type="datetime-local"
                value={closedAt}
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full rounded-lg bg-emerald-600 py-2.5 text-sm font-semibold text-white hover:bg-emerald-500 disabled:opacity-50 transition-colors cursor-pointer"
            >
